...
{#if retainsContent}
  <div class="surface-anatomy" data-surface-presentation={fixture.presentation}>
    ...
    {/if}
    {#if fixture.presentation !== 'theme'}
      <p class="surface-example-notice" data-static-example>Static example · controls do not change your settings or saved data.</p>
    {/if}
    ...
    {:else if fixture.presentation === 'composer'}
      <div class="widget-content composer" data-conformance-region="composer">
        ...
        <button disabled type="button" data-pom-part="button.surface">Send action</button>
      </div>
    {:else}
      <div class="implemented-surface">
        ...
        {:else if fixture.presentation === 'ambience'}
          <div class="surface-mixer">
            {#each fixture.rows as row, index (row[0])}
              ...
              <label><span>{row[0]}</span><output>{row[1]}</output><input data-pom-part="slider.input" aria-label={`${row[0]} level`} type="range" min="0" max="100" value={level} style={sliderStyle(level, 0, 100)} disabled /></label>
            {/each}
          </div>
        ...
        {:else if fixture.presentation === 'archive'}
          <div class="surface-workspace">
            <nav aria-label="Library selection">
              {#each fixture.rows as row, index (row[0])}<button disabled type="button" data-pom-part="button.surface" aria-current={index === 0 ? 'true' : undefined}>{row[0]}<small>{row[1]}</small></button>{/each}
            </nav>
        ...
        {:else if fixture.presentation === 'roster'}
          <div class="surface-roster">
            {#each fixture.rows as row (row[0])}<button disabled type="button" data-pom-part="button.surface"><span class="surface-avatar">{initials(row[0])}</span><strong>{row[0]}</strong><small>{row[1]}</small><i aria-hidden="true"></i></button>{/each}
          </div>
        {:else if fixture.presentation === 'wizard'}
          <ol class="surface-wizard" aria-label="Reviewed workflow">
            {#each fixture.rows as row, index (row[0])}<li data-pom-part="row.surface"><span>{index + 1}</span><div><strong>{row[0]}</strong><small>{row[1]}</small></div><button disabled type="button" data-pom-part="button.surface">Choose</button></li>{/each}
          </ol>
        {:else if fixture.presentation === 'document'}
          <div class="surface-document">
            {#each fixture.rows as row (row[0])}<label>{row[0]}<input readonly data-pom-part="field.surface" value={row[1]} aria-label={row[0]} /></label>{/each}
            <label class="surface-document-body">Notes<textarea readonly data-pom-part="field.surface">Rain-softened maps and a careful record of the third bell.</textarea></label>
          </div>
        {:else if fixture.presentation === 'tree'}
          <div class="surface-tree"><button disabled type="button" data-pom-part="button.surface" aria-expanded="true">▾ Drowned Observatory</button><button disabled type="button" data-pom-part="button.surface" aria-expanded="true">　▾ Thresholds</button><button disabled class="is-selected" type="button" data-pom-part="button.surface">　　 The pale threshold</button><button disabled type="button" data-pom-part="button.surface">　 Floodgate history</button></div>
        ...
        {:else if fixture.presentation === 'credentials'}
          <div class="surface-providers">
            {#each fixture.rows as row, index (row[0])}<div data-pom-part="row.surface"><span class="surface-avatar">{row[0].slice(0, 2).toUpperCase()}</span><strong>{row[0]}</strong><small>{row[1]}</small><button disabled type="button" data-pom-part="button.surface">{index === 1 ? 'Configure' : 'Test'}</button></div>{/each}
          </div>
        {:else if fixture.presentation === 'assignments'}
          <div class="surface-assignments">
            {#each fixture.rows as row (row[0])}<label data-pom-part="row.surface"><span>{row[0]}<small>{row[1]}</small></span><select disabled data-pom-part="field.surface" aria-label={`${row[0]} assignment`}><option>{row[1]}</option><option>Follow Default</option></select></label>{/each}
          </div>
        {:else if fixture.presentation === 'theme'}
          <div class="surface-themes">
            {#each hostContext.theme.presets as preset (preset.id)}<button type="button" data-pom-part="button.surface" data-pom-control-shape="content-tile" aria-pressed={hostContext.theme.activeId === preset.id} onclick={() => hostContext.theme.activate(preset.id)}><i style={preset.swatchStyle}></i><strong>{preset.label}</strong><small>{preset.description}</small></button>{/each}
          </div>
        {:else if fixture.presentation === 'sound-motion'}
          <div class="surface-accessibility">
            <label><input data-pom-part="field.surface" type="checkbox" checked disabled /> Interface sounds</label>
            <label>Animation <select data-pom-part="field.surface" disabled><option>Reduced</option></select></label>
            <label>Autoplay ambience <select data-pom-part="field.surface" disabled><option>Ask first</option></select></label>
          </div>
        {:else if fixture.presentation === 'accessibility'}
          <div class="surface-accessibility">
            <label>Text scale <input data-pom-part="slider.input" type="range" min="80" max="160" value="100" style={sliderStyle(100, 80, 160)} disabled /><output>100%</output></label>
            <label><input data-pom-part="field.surface" type="checkbox" disabled /> High contrast</label><label><input data-pom-part="field.surface" type="checkbox" disabled checked /> Follow reduced-motion setting</label>
            <p aria-label="Accessibility sample">Sample: Reservoir light across quiet water.</p>
          </div>
        {:else if fixture.presentation === 'prompt'}
          <div class="surface-prompt"><nav aria-label="Prompt sheets"><button disabled data-pom-part="button.surface" aria-current="page">Narration</button><button disabled data-pom-part="button.surface">Director</button><button disabled data-pom-part="button.surface">Characters</button><button disabled data-pom-part="button.surface">Memory</button></nav><label>Prompt sheet<textarea data-pom-part="field.surface" readonly>You are the narrator. Preserve established facts, subjective knowledge, and player agency.</textarea></label></div>
        {:else}
          <dl class="surface-facts">
        ...
        {#if fixture.actions.length}
          <footer class="surface-actions">{#each fixture.actions as action (action)}<button type="button" data-pom-part="button.surface" disabled={action !== 'Open Custom Theme'} onclick={action === 'Open Custom Theme' ? hostContext.theme.openSettings : undefined}>{action}</button>{/each}</footer>
        {/if}
      </div>
    {/if}
    {#if fixture.presentation === 'reader' || fixture.presentation === 'composer'}
      ...
      {#if fixture.actions.length && fixture.presentation !== 'composer'}
        <footer class="surface-actions">{#each fixture.actions as action (action)}<button disabled type="button" data-pom-part="button.surface">{action}</button>{/each}</footer>
      {/if}
    {/if}
  </div>
{/if}
