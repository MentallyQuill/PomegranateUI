...
describe('WidgetCatalog', () => {
  it('recovers a failed unknown-icon render without residual catalog lifecycle state', async () => {
    ...
    expect(lifecycle.mutationDisconnect).toHaveBeenCalledTimes(98);
  ...
  it('isolates later invalid structural snapshots without registering a precommit owner', async () => {
    ...
    expect(before.mutationObservers).toBe(98);
    ...
    expect(before.results).toBe(98);
    expect(before.previews).toBe(98);
    expect({
    ...
    expect(rendered.container.querySelectorAll('[data-catalog-result]')).toHaveLength(98);
    expect(rendered.container.querySelectorAll('svg[data-catalog-icon] use')).toHaveLength(98);
  ...
  it('resolves all 98 manifests to the expected 13 source symbols in Visual and Compact modes', async () => {
    const user = userEvent.setup();
    const manifests = createCatalogManifests();
    const { container } = renderCatalog();
    const iconKeys = new Set(manifests.map((manifest) => manifest.catalog!.iconKey));

    expect(manifests).toHaveLength(98);
  ...
  it('renders the fixed source controls and all 98 whole-result shared previews', async () => {
    ...
    expect(within(dialog).getByText('98 widgets')).toBeVisible();
  ...
  it('measures intrinsic content and lets only the newest rapid restack restore its anchor', async () => {
    ...
    const drainFrames = () => {
      for (let remaining = 20; pendingFrames.size > 0 && remaining > 0; remaining -= 1) {
        executeFrame([...pendingFrames][0]!);
      }
      if (pendingFrames.size > 0) throw new Error('Catalog anchor restoration did not settle.');
    };
    vi.stubGlobal('requestAnimationFrame', requestFrame);
    vi.stubGlobal('cancelAnimationFrame', cancelFrame);
    const user = userEvent.setup();
  ...
  it('uses Compact as the same one-column result tree without previews or preview sizing', async () => {
    ...
    expect(container.querySelectorAll('[data-catalog-result]')).toHaveLength(98);
