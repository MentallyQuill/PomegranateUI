<script lang="ts">
  ...
  const catalogContexts: Readonly<Record<string, string>> = Object.freeze({
    ...
    'settings.theme-colors': 'global-device',
    'settings.theme-materials': 'global-device',
    'settings.theme-canvas': 'global-device',
    'settings.theme-ambient': 'global-device',
  ...
  let retainedRestackAnchor: CatalogScrollAnchor | null = null;
  let destroyed = false;
  ...
  function retainedAnchorIsCurrent(anchor: CatalogScrollAnchor): boolean {
    if (!resultsElement) return false;
    const result = getResults().find((candidate) => candidate.dataset.widgetType === anchor.key);
    if (!result) return false;
    const regionRect = resultsElement.getBoundingClientRect();
    const resultRect = result.getBoundingClientRect();
    const visible = resultRect.bottom > regionRect.top && resultRect.top < regionRect.bottom;
    return visible && Math.abs(resultRect.top - regionRect.top - anchor.offset) <= 1;
  }

  function restack(update: () => void) {
    const anchor = retainedRestackAnchor && retainedAnchorIsCurrent(retainedRestackAnchor)
      ? retainedRestackAnchor
      : gridController?.captureAnchor() ?? null;
    retainedRestackAnchor = anchor;
    ...
    void tick().then(() => {
      if (destroyed || revision !== restackRevision) return;
      gridController?.sync();
      const settle = (stableFrames: number, attempts: number) => {
        const frame = requestAnimationFrame(() => {
          ...
          gridController?.sync();
          const correction = gridController?.restoreAnchor(anchor) ?? 0;
          const nextStableFrames = Math.abs(correction) <= 0.5 ? stableFrames + 1 : 0;
          if (nextStableFrames >= 2 || attempts >= 7) {
            if (pendingRestoreFrame === frame) pendingRestoreFrame = null;
            return;
          }
          settle(nextStableFrames, attempts + 1);
        });
        pendingRestoreFrame = frame;
      };
      settle(0, 0);
