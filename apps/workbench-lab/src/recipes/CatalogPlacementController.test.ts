...
describe('CatalogPlacementController', () => {
  ...
  it('releases touch pointer capture before hiding the origin and still cancels on window blur', () => {
    vi.useFakeTimers();
    ...
    controller.pointerDown(pointerEvent('pointerdown', { clientX: 10, clientY: 10, pointerType: 'touch' }), manifest, origin);
    expect(setPointerCapture).toHaveBeenCalledWith(1);
    vi.advanceTimersByTime(300);
    expect(controller.getState().phase).toBe('lifted');
    expect(releasePointerCapture).toHaveBeenCalledWith(1);
    origin.dispatchEvent(pointerEvent('lostpointercapture', { clientX: 10, clientY: 10, pointerType: 'touch' }));
    expect(controller.getState().phase).toBe('lifted');
  ...
  it('retains mouse capture through lift so the synthesized click stays on the origin', () => {
    const { root, origin } = placementSurface();
    const releasePointerCapture = vi.fn();
    Object.defineProperties(origin, {
    ...
    document.dispatchEvent(pointerEvent('pointerup', { clientX: 16, clientY: 10 }));
    expect(releasePointerCapture).toHaveBeenCalledOnce();
    controller.destroy();
  });
  ...
  it('preserves touch scrolling before the hold and suppresses native scrolling after lift', () => {
    vi.useFakeTimers();
    const { root, origin } = placementSurface();
    const controller = createCatalogPlacementController({
      catalog: { suspend: vi.fn(), resume: vi.fn() },
    ...
    const beforeLift = new Event('touchmove', { bubbles: true, cancelable: true });
    document.dispatchEvent(beforeLift);
    expect(beforeLift.defaultPrevented).toBe(false);

    vi.advanceTimersByTime(300);
    const afterLift = new Event('touchmove', { bubbles: true, cancelable: true });
    document.dispatchEvent(afterLift);
    expect(afterLift.defaultPrevented).toBe(true);
