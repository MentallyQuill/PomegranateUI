<script lang="ts">
  ...
  import { diagnosticsFor, editableThemeDraft, type ThemeAuthoringPort } from './types.js';
  ...
  function updatePosition(position: { x: number; y: number }) {
    ...
    next.ambient.position = position;
    theme.editDraft(next);
  }

  function updateValue(field: 'radius' | 'power', value: number) {
    const next = editableThemeDraft(theme);
    next.ambient[field] = value;
    theme.editDraft(next);
  }
</script>
...
<div class="theme-authoring-element theme-authoring-ambient" data-theme-authoring-element="ambient">
  ...
  <div class="theme-authoring-ranges">
    {#each [['radius', 'Radius'], ['power', 'Power']] as control (control[0])}
      <label>
        <span>{control[1]}</span>
        <output>{Math.round(draft.ambient[control[0] as 'radius' | 'power'] * 100)}%</output>
        <input
          ...
          min="0"
          max="100"
          value={Math.round(draft.ambient[control[0] as 'radius' | 'power'] * 100)}
          style={`--pom-slider-progress:${compileSliderProgress(draft.ambient[control[0] as 'radius' | 'power'] * 100, 0, 100)}`}
          oninput={(event) => updateValue(control[0] as 'radius' | 'power', Number(event.currentTarget.value) / 100)}
        />
      </label>
    {/each}
  </div>
  {#if diagnostics.length}
    <ul class="theme-authoring-diagnostics" aria-label="Ambient diagnostics">
