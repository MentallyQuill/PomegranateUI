...
describe('implemented Deep Current surface boundary', () => {
  ...
  it('freezes all 94 reviewed Widget identities and exact family totals', () => {
    expect(IMPLEMENTED_SURFACES).toHaveLength(94);
    expect(new Set(IMPLEMENTED_SURFACES.map(({ type }) => type)).size).toBe(94);
    expect(IMPLEMENTED_SURFACE_TOTALS).toEqual({ story: 12, library: 19, systems: 21, settings: 39, extensions: 3 });
