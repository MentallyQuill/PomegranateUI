...
strip always wins tab intent. Shelf rails use real gaps where available; the
first rail may begin at the Widget-body edge when no gap exists above it, but
must never cover the title/tab strip.
Across the remaining Widget body, the top quarter activates Shelf above, the
middle half remains a tab target, and the bottom quarter activates Shelf below.
The preview remains attached to the pointer, and neighboring tabs/shelves move
to show the resulting arrangement before release. If less than the standard
preview height remains below the final shelf, compress the preview into that
space rather than drawing it back over Widget content or footer controls.
