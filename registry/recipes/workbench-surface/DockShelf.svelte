<script lang="ts">
  ...
  let { projection, store, renderWidget, resizable = false }: { projection: PanelShelfProjection; store: WorkbenchStore; renderWidget: Snippet<[WidgetFrameProjection]>; resizable?: boolean } = $props();
...
<section class="dock-shelf" data-pomegranate-shelf={projection.shelf.id} data-pomegranate-shelf-order={projection.shelf.order} data-pom-part="dock.surface" aria-label={`${projection.shelf.id} shelf`} style={`--pom-shelf-weight:${projection.shelf.weight}`}>
  ...
  {#each groups as groupId (groupId)}
    <WidgetGroup frames={projection.frames.filter((frame) => frame.placement.kind === 'docked' && frame.placement.group?.id === groupId)} {store} {renderWidget} />
  {/each}
</section>
