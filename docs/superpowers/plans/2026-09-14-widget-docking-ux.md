...
- [x] Run `test:unit`, `typecheck`, `test:native`, `build`, `check:recipes`, `test:pack`, `test:browser` and `check` as applicable; inspect coverage, not just exit codes.
- [x] Exercise Catalog, docked, grouped, floating and shelved widgets across all themes and responsive states. Include cancellation, restoration, undo, persistence and actual animation frames.
- [x] Review scoped diff and recipe parity. Record limitations; do not claim completion if any requirement lacks evidence.
...
- Final audit found two invalid Windows-1252 ellipsis bytes in the copy-owned action recipes. A fatal UTF-8 decoding regression failed before repairing the bytes; all eight recipe unit cases and the six-package/two-clean-consumer build passed afterward. Recipe revisions in the unit contract now match the deliberately upgraded manifests.
- Final browser audit caught a detached widget preview retaining its former group's other tab names. The held snapshot now retains only the dragged tab, including an inactive source, while the live group's selection remains unchanged after cancellation. The original browser regression passes. Older tests now verify visible actions, keyboard-reachable clipped rails, noninteractive collapsed overlays, direct empty-column drops, stable widget boundary cues and completed arrival geometry.
- Visual review rejected the first short-landscape/zoom baseline: the transcript overlapped the story title, and flow layouts reserved composer space twice. Four new short-landscape cases reproduced title overlap across every theme. Their final fix and baseline verification are pending below.
- A full-width widget insertion zone also trapped the pointer over an append shelf rail. The new native regression failed before giving the rail's interior priority; its edges retain hysteresis. All 13 native docking cases pass. Cross-panel persistence and the full browser rerun are pending below.
- Current visual baselines intentionally reflect visible action/header surfaces, the persistent Shelf and compact Undo controls, and content-derived shelf allocation. The held-widget golden no longer hides its contents through screenshot-only CSS. Historical Atmospheric authority images remain untouched.
- Final fixes verified: the Scene/Library/Settings persistence case now reaches its append rail and passes; Catalog body insertion stays on the indicated shelf at the correct widget order. Detached inactive-widget cancellation retains live selection. All four short-landscape cases now pass after separating the story title, reclaiming double-reserved composer space and removing the unified composer's negative translation in short windows. Prose remains scrollable, with at least 64px of readable content and no composer overlap. Screenshots were inspected for Deep Current, PomOS and Ash & Amber, with the cross-theme geometry checks covering Bunny as well.
- The visual/geometry pass covered 77 cases; its sole outstanding Ash composer overlap was subsequently fixed and all four short-landscape regressions passed. All 45 changed visual baselines were reviewed as contact sheets or full images, including the data-only external theme and recognizable held content. The clean final `check` has passed 52 unit tests, typecheck without errors/warnings, 652 native tests in 61 files, build, artifact/recipe checks and six-package/two-clean-consumer verification. Its 327-case browser run is pending below.
- Final result: `npm.cmd run check` passed every non-browser gate and completed the entire browser suite with 326 passed and one external-theme screenshot timing failure (9.8 minutes). That static fixture changed its theme while compositor/observer geometry was settling. It now uses reduced motion from setup and waits for fonts plus layout frames; its reviewed baseline passed three consecutive normal comparison reruns. The aggregate command was not rerun after this test-only correction; all 327 browser cases have passing evidence, with motion-enabled interaction coverage unchanged. Logs: `final-check.log`, `external-fixture-verify.log` (ignored local evidence).
- Coverage includes all four maintained themes, data-only external theme rendering, phone/short-landscape/desktop/zoom-equivalent layouts, Catalog/existing/grouped/floating/Shelf placements, preview-to-commit geometry, reflow and arrival frames, interrupted motion, reduced motion, mouse/keyboard and emulated pen/touch, cancellation, Undo, restoration and explicit save/reload. Physical touch/pen hardware and additional browser engines were not tested. Forty-five updated Windows visual goldens and the final scoped diff were inspected; recipe parity and fatal UTF-8 validation passed.
- Delivery: all seven items and the final audit are complete on `codex/widget-docking-ux` in the isolated worktree. Parent checkout changes were preserved. No merge, push or package publication is included.
- Authorized integration: reconcile `origin/main` at `2da2961` while preserving its live focused-widget actions, surface context menus, themed toolbar controls, overflow containment and Story resize fixes. Consolidate content-count shelf sizing with gap-safe fractional rows and the shared Catalog floating bounds. The incoming one-move floating regression exposed a missing initial target calculation; calculating intent immediately on lift passes all 51 focused controller/docking tests. Typecheck and all 657 native tests pass; complete combined-branch verification is running before the requested PR merge.
- Integration audit: reviewed 25 refreshed combined-layout screenshots, aligned the copied shelf count contract and semantic button foreground assertions, and verified all 31 affected visual/browser rechecks plus all 24 theme UX tests. Windows CI exposed a delayed menu-opening callback that could steal focus from Move to Move tab right before Enter, unintentionally reordering the group. A controlled callback regression failed before preserving in-menu focus and passed afterward. All 59 action, docking and theme-renderer browser cases pass with that fix, as do typecheck/build and three native action tests. Arrival geometry now samples the real paused browser animations at fixed times rather than requiring the runner to render their final 40ms; three repeated probes and the full affected browser pass retain endpoint, reflow and cleanup assertions. The full persistence round trip has a scoped 15-second native-test budget for Windows CI. Final Windows/Linux checks are recorded on PR #23.
