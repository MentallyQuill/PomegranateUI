...
import { renderAshAmberReference } from '../drivers/reference/ash-amber.ts';
...
import { ASH_AMBER_SCENARIOS, hashAuthorityFile, validateConformanceManifest } from '../manifest.ts';
...
const ledgerPath = path.join(repositoryRoot, 'docs', 'conformance', 'ash-amber-ledger.md');

test.describe('Ash & Amber target conformance', () => {
  test.describe.configure({ mode: 'serial' });

  test.beforeAll(async () => {
    await validateConformanceManifest(ASH_AMBER_SCENARIOS, {
      repositoryRoot,
  ...
  for (const scenario of ASH_AMBER_SCENARIOS) {
    test(scenario.id + ' ' + scenario.title, async ({ browser }, testInfo) => {
      ...
      if (!viewport || !profile) throw new ConformanceError('MANIFEST_INVALID', 'Missing Ash & Amber runtime profile for ' + scenario.id + '.');
      const ledger = parseDiscrepancyLedger(await readFile(ledgerPath, 'utf8'));
      validateDiscrepancyLedger(ledger, ASH_AMBER_SCENARIOS);
      const paths = createEvidencePaths(outputDirectory, scenario.id);
      await mkdir(paths.directory, { recursive: true });
      const contextOptions = {
        viewport,
        hasTouch: scenario.viewport === 'compact-small',
        isMobile: scenario.viewport === 'compact-small'
      };
      ...
      try {
        ...
        const reference = await renderAshAmberReference(referencePage, preservationOrigin, scenario);
        ...
        await writeComparisonReport(paths, {
          authorityCase: 'ash-amber ' + scenario.referenceState + ' semantic reference',
          comparison,
          diagnosticImages,
          discrepancyIds: [],
