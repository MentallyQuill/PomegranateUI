...
import { prepareWidgetOverhaulSurfacePage, renderWidgetOverhaulSurface, requireSurfaceHarnessCase } from '../drivers/reference/widget-overhaul-surfaces.ts';
import { renderLabWidgetSurface } from '../drivers/workbench-lab/widget-surfaces.ts';
...
import { DEEP_CURRENT_WIDGET_SCENARIOS, hashAuthorityFile, validateConformanceManifest } from '../manifest.ts';
...
import { DEEP_CURRENT_WIDGET_SURFACES } from '../widget-manifest.ts';
...
test.describe('Deep Current Widget surface conformance', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    await validateConformanceManifest(DEEP_CURRENT_WIDGET_SCENARIOS, {
      repositoryRoot,
    ...
    validateDiscrepancyLedger(ledger, DEEP_CURRENT_WIDGET_SCENARIOS);
    const viewport = CONFORMANCE_VIEWPORTS.get('standard');
    if (!viewport) throw new ConformanceError('MANIFEST_INVALID', 'The standard Widget viewport is missing.');
    ...
    await prepareWidgetOverhaulSurfacePage(referencePage, preservationOrigin);
  });
  ...
  for (const scenario of DEEP_CURRENT_WIDGET_SCENARIOS) {
    test(`${scenario.id} ${scenario.title}`, async ({}, testInfo) => {
      const surfaceCase = DEEP_CURRENT_WIDGET_SURFACES.find(({ scenarioId }) => scenarioId === scenario.id);
      const profile = MEASUREMENT_PROFILES.get(scenario.measurementProfile);
      if (!surfaceCase || !profile) throw new ConformanceError('MANIFEST_INVALID', `Missing Widget runtime profile for ${scenario.id}.`);
      const paths = createEvidencePaths(outputDirectory, scenario.id);
      await mkdir(paths.directory, { recursive: true });

      const authorityCase = requireSurfaceHarnessCase(referenceCases, surfaceCase);
