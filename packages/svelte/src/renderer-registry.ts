...
export type WidgetRenderer<THostContext> = Component<WidgetRendererProps<THostContext>>;

export type RendererRegistrationResult<THostContext> =
  | {
      readonly ok: true;
      readonly type: WidgetType;
      readonly renderer: WidgetRenderer<THostContext>;
    }
...
export interface WidgetRendererRegistry<THostContext> {
  register(
    type: WidgetType,
    renderer: WidgetRenderer<THostContext>
  ): RendererRegistrationResult<THostContext>;
  unregister(type: WidgetType): boolean;
  get(type: WidgetType): WidgetRenderer<THostContext> | undefined;
...
export function createWidgetRendererRegistry<THostContext>(): WidgetRendererRegistry<THostContext> {
  const renderers = new Map<WidgetType, WidgetRenderer<THostContext>>();
    ...
    register(
      ...
      if (renderers.has(type)) {
        return Object.freeze({
          ok: false as const,
          error: Object.freeze({
            code: 'DUPLICATE_RENDERER' as const,
            message: `Renderer for Widget type '${type}' is already registered.`
          })
        });
      }
      renderers.set(type, renderer);
      return Object.freeze({ ok: true as const, type, renderer });
    },
    unregister(type: WidgetType): boolean {
